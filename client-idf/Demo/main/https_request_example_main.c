...
{
    ESP_LOGI(TAG, "【w】https_request using crt bundle");
    esp_tls_cfg_t cfg = {
...
{
    ESP_LOGI(TAG, "【w】https_request using cacert_buf");
    esp_tls_cfg_t cfg = {
...
#ifdef CONFIG_EXAMPLE_CLIENT_SESSION_TICKETS//未参与编译   Session Ticket 机制（RFC 5077）用于在 TLS 连接中缓存会话信息，以便后续请求可以复用，提高握手速度。
...
#if CONFIG_MBEDTLS_CERTIFICATE_BUNDLE && CONFIG_EXAMPLE_USING_ESP_TLS_MBEDTLS
    https_get_request_using_crt_bundle();//使用证书包进行 HTTPS 请求
#endif
    ESP_LOGI(TAG, "Minimum free heap size: %" PRIu32 " bytes", esp_get_minimum_free_heap_size());
  //  https_get_request_using_cacert_buf();//使用 CA 证书缓冲区 进行 HTTPS 访问，适用于设备存储空间有限的情况
 //   https_get_request_using_global_ca_store();//使用全局 CA 证书存储，多个请求可共享同一套 CA 证书，减少 RAM 占用。
  //  https_get_request_using_specified_ciphersuites();//指定 TLS 密码套件 进行 HTTPS 连接，适用于对安全性或兼容性有特殊要求的应用。
